...
> **Progress — Phase 3 (started 2026-07-22):** 🚧 in progress on branch `acefuels-phase3-campaigns`. ✅ **F2 notification-targeting backbone + F3 auto-milestone shipped (backend + web + API), tested (481 Rails runs green, brakeman clean).** **Foundation:** `notification_messages` + `notification_recipients` (per-person, per-channel delivery log — the persistent record the ephemeral push result never was); `customers` += `whatsapp_opt_in`/`sms_opt_in`/`last_milestone_points`; `push_subscriptions` += `consent_at`. **Engine:** `Notifications::AudienceResolver` (target → candidate customers; `all` = every token incl. anonymous for push / every opted-in for WA/SMS), a `NotificationChannel` abstraction — `PushChannel` (reuses one FCM token per dispatch, skips cleanly when unconfigured) + `WhatsappChannel`/`SmsChannel` (record `skipped` until a real provider is wired — never silently sent), `Notifications::Dispatcher` (fan-out + per-channel opt-in/token gating + counts), `Notifications::Broadcaster` (the shared send entry point). `FirebasePushService` gains a public `deliver_one` + an offer `data` block. **F2 send (web + API):** `POST /api/v1/admin/notifications/send` now takes `channels[]`/`target_type`/`target_customer_type`/`customer_ids[]`/`category` and returns per-channel counts; `GET /notifications` history + `GET /notifications/:id/recipients`; the web "Send Now" form gains an Audience selector + Push/WhatsApp/SMS checkboxes + a Delivery-history table. **F3:** `LoyaltyMilestoneNotifier` fires one auto "you've earned N points" notification per rung (`reward_settings.milestone_step`, default 500) after `TransactionCreator` commits, idempotent via `customers.last_milestone_points`. ✅ **F1 Campaigns (backend + web + API), adversarially reviewed + tested:** `campaigns`/`campaign_targets`/`campaign_qualifications`; `Campaigns::Evaluator` (window aggregation → qualifiers), `Campaigns::Runner` (idempotent grant + offer dispatch), `Campaigns::Preview` (dry run); reward kinds discount/gift/bonus-points, ₹ **and** litre thresholds, targeting all/type/individual/selected; `GET/POST/PATCH/DELETE /api/v1/admin/campaigns` (+ preview/run/activate/pause) + `Admin::Campaigns` web console. A 5-dimension multi-agent adversarial review caught + fixed a **rolling-window daily double-grant** (idempotency key now a stable anchor), **network-IO-inside-the-lock** (claim-then-send: grant under lock, dispatch outside), a **status guard** (only active campaigns grant), and an **N+1 reachability** query. ✅ **F2 reachability:** WhatsApp/SMS opt-in checkboxes on the customer form (both surfaces) + `push_subscriptions.consent_at` stamped on phone-linked registration. **508 Rails runs green, brakeman clean.** ✅ **Android lockstep (partial):** the native **campaigns** admin area (`ui/admin/campaigns` — list/detail + preview/run/activate/pause) and **customer opt-in toggles** (WhatsApp/SMS on the profile, via a new staff customer-update endpoint) shipped; compiles clean. ✅ **A7 Operator KYC (backend + API + web):** ActiveStorage + `has_one_attached :profile_photo/:id_card_photo`; `users` += `address` + Aadhaar (**Active Record Encryption** at rest + **Verhoeff** checksum + masked `XXXX-XXXX-1234` via `aadhaar_last4`); masked-by-default serializer + an admin-only **audited** reveal (`pii_access_logs`) + authenticated ID-card view + Purge-KYC + Aadhaar log-filtering; multipart create/update; password login unchanged (Q3). Tested (524 green, brakeman clean). *(Production must provision GCS durable storage + AR-encryption keys in Secret Manager.)* ✅ **Android A7 KYC capture UI shipped** (admin Users sheet: Address + Aadhaar [12-digit, checksum server-verified] + Profile/ID-card photos via a Take-photo [camera + FileProvider, runtime-permission-gated] or gallery chooser → multipart upload with a JSON fallback for no-image edits; audited full-Aadhaar reveal + signed ID-card open + Purge-KYC; Coil thumbnails on the shared authed OkHttp client; compiles clean). ✅ **Android notification delivery-history screen shipped** (`ui/admin/notifications` — read-only `GET /notifications` list with per-channel sent/skipped/failed tallies + audience/when/by, tap → per-recipient sheet from `GET /notifications/:id/recipients`; reached via a History action on the Notifications top bar) — this + the earlier native notification-targeting send UI (5ad61af) **complete Phase 3 Android lockstep**. ✅ **Public `/loyalty/result` opt-in card shipped (2026-07-22):** a "Get offers & bonus alerts" card — the reused push panel passes the looked-up phone so `POST /push/subscriptions` links + stamps `consent_at`, plus WhatsApp/SMS opt-in checkboxes → a new CSRF-protected `loyalty#opt_in` (phone derived from the signed lookup token, never a client param); English `loyalty.optin.*` copy + tested. ✅ **A7 production prerequisites wired (2026-07-22):** `google-cloud-storage` gem + a `:google` ActiveStorage service (Cloud Run ADC, no keyfile) + `ACTIVE_STORAGE_SERVICE`-driven `production.rb`; `cloudbuild.yaml` mounts the `AR_ENCRYPTION_*` secrets + `GCS_*` env into the migrate job and service; operator scripts `setup-encryption-keys.sh` (never overwrites) + `setup-gcs.sh` (private bucket) + runbook `docs/a7-prod-provisioning.md`. ✅ **Schedule channel/target extensions shipped (2026-07-22):** `notification_schedules` += `channels`/`target_type`/`target_customer_type`/`campaign_id` (migration `20260722190000`); the runner + send-now route through `Notifications::Broadcaster` (category `scheduled`) — targeted, multi-channel, logged, still lease-guarded/idempotent; web schedule form + Android editor gained channel multi-select + audience selector; serializers/DTOs carry the fields; 547 Rails green + Android compiles. ✅ **Phase 3 MERGED + DEPLOYED to prod (2026-07-22):** operator provisioned AR-encryption keys + GCS; main → rev `fuel-loyalty-git-00076`, healthy. *Next (post-Phase-3): **F4 SMS** (MSG91 DLT) deferred by decision.*

> **Progress — Phase 4 (2026-07-22):** ✅ **COMPLETE + MERGED + DEPLOYED to prod** (main `e0fd454`, Cloud Build `a1984626` SUCCESS, revision `fuel-loyalty-git-00079` healthy; the 2 new migrations ran and the CRM routes are live). ✅ **CRM intelligence — E3/E5/E6/E7 shipped on all three surfaces, tested (620 Rails runs green incl. review fixes, brakeman clean, Android `assembleDebug` SUCCESSFUL).** **Foundation:** `contact_logs` (E5 — an outreach *event* log: who/channel/outcome/when, distinct from B1's `customer_contacts` *people* roster; a reached/converted/callback outcome flips the linked B1 contact's `contacted` marker via `Admin::Crm::ContactLogRecorder`) + `customer_feedbacks` (E7 — 1–5 rating + comment, optional visit/transaction link, source staff/admin, one-per-artifact partial-unique indexes). No denormalized visit columns on `customers` — cadence/churn are **computed on read** from `Customer.visited_between`, a **union of `transactions.created_at` + `visit_entries.entry_date`** (so neither the loyalty-only drive-in segment nor the visit-only fleet/OTP/credit segment is dropped). **Engine (pure + testable):** `Admin::Crm::Cadence` (median inter-visit gap over trailing 90d → daily ≤2 / weekly ≤10 / biweekly ≤21 / monthly ≤45 / occasional / new-for-<3-visits, + expected-next), `Admin::Crm::ConversionScore` (transparent 0–100 recency+frequency+last-outcome blend, clamped), `Admin::Crm::CustomerInsight` (E3/E5 per-customer profile + contact/feedback rollups), `Admin::Crm::ChurnReport` (E6 — visited-previous-period-not-current, set-based + batch-loaded, ranked by conversion probability). **API:** `GET /api/v1/admin/customers/:id/insight`, `GET/POST /api/v1/admin/customers/:id/contact_logs`, `GET/POST /api/v1/admin/customers/:id/feedbacks` (also under `/api/v1/staff/...` so FSMs capture ratings at the pump), `GET /api/v1/admin/dashboard/churn`. **Web:** CRM Insight / Outreach / Feedback cards + log-contact & add-feedback modals on the admin customer page; a new **Reach out** admin page (churn list) + sidebar entry. **Android:** new `ui/admin/crm` package (Api/Dtos/Repository + `ReachOutScreen`/VM + `CustomerCrmViewModel`); a Feedback section (star capture) for all users + admin-only CRM Insight (cadence chip + conversion gauge) and Outreach (log list + log-contact sheet) on the shared customer profile; a **Reach out** Ops tile + nav route. **81 new tests** (model, service-math, admin+staff request specs, web controllers) — including 8 from a 5-dimension adversarial review that confirmed + fixed 4 findings (a HIGH IDOR on feedback record-linking, a conversion-score inconsistency for 2-visit customers, and two 500→422/409 error-mapping gaps). **Phase 4 completes the requirement's intelligent-dashboard vision** — only F4 live WhatsApp/SMS (deferred) and G1's per-pump transaction filter + past-day transaction edit remain across the whole roadmap.*

> **Progress — Admin review round (2026-08-19):** ✅ **Five items of admin feedback shipped across web, API and Android, tested (703 Rails runs green).** **1 — My Pump is always today.** The date picker is gone (web + API) for admins as well as staff: a write with any other date is refused (`422 daily_assignment_only`), a read serves today and echoes `assignment_date` so a stale client corrects itself. Back-dating cannot move transactions that already snapshotted their pump, so it only ever *looked* as though it did; the admin's Assign Pump screen still plans ahead but refuses past dates (`422 past_assignment_date`). Android drops the self-service date field and re-reads the clock on load. *(This supersedes S-MYPUMP as originally specified — My Pump is self-service for both roles and lives in the shared Access nav, not under Admin; admins need it because `TransactionCreator` refuses to record for anyone without a pump + nozzle. See `14-spec-rewards-staff-constraints.md`.)* **2 — Admin > Users** lists active accounts first and shows an Active/Inactive badge the list never carried. **3 — Settlements are reported and attributed per staff member (Admin-12).** The admin console filters by FSM and rolls each day up per FSM (count, pumps covered, money columns; drafts counted separately so the row cannot contradict its own totals; grouped on `recorded_by_id`, never the frozen name snapshot). An admin who **edits** must name the staff member the edit is for (`settlement_changes.on_behalf_of`, beside `changed_by`); an admin who **creates** must name the FSM it belongs to (`daily_settlements.entered_by` records who keyed it in, stamped once at creation and never back-stamped). Neither may name the acting admin, ownership can never be re-pointed after creation, and reconcile is exempt — approving is the admin's own authority. Android drops its Daily Settlement tile for admins, matching the web, since its request body carries no FSM field. Two migrations. **4 — Admin > Customers** filters on *at least* N visits / litres filled / times contacted / ₹ discount given / reward points, and shows those five figures on each row. The period filter now means "served in this window", so fleet/OTP customers with visit captures but no loyalty transaction are no longer dropped. **5 — A customer's profile reports litres filled, discount given and gifts given** (₹ value of redemptions), on the web and in `GET /admin/customers/:id/insight`, period-scoped with lifetime figures beside them. Items 4 and 5 share **one** SQL definition (`Admin::Crm::CustomerMetrics`) so the list and the profile can never disagree: visits are distinct **days** (matching the cadence figure already shown), litres/discount are every transaction plus only those `visit_entries` with no linked transaction (a linked pair carries identical figures on both rows, so summing both doubles everything), flows are period-scoped while the points balance stays lifetime. A 4-dimension adversarial review raised 7 defects, all fixed with regression tests — the worst being `entered_by` back-stamping onto every pre-migration settlement at the next edit or reconcile, which would have mis-attributed the whole settlement history to whichever admin reconciled it. **Also fixed, pre-existing on main:** `AttendanceRosterBuilder#call` raised `NoMethodError` on every request that reached the admin attendance planner (it called `assignment.user` on `User` records); `Settlement::Builder` resolved the pump *before* parsing the business date and with no date at all, so a back-dated draft pre-filled another pump's opening readings and pulled that pump's discount lines; and two tests left stale by earlier deliberate changes (optional vehicle details in the transaction modal, and the staff customer list becoming an A–Z directory rather than a top-3-by-points leaderboard). **Suite now fully green: 703 runs, 0 failures.**
