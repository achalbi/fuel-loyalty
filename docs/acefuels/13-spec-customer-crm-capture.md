...
> **Implementation status (2026-07-22):** 🟡 **B1/E4 backend + web shipped, tested.** `customers` gained `customer_type` (drive_in/otp/credit enum, backfilled), `transport_name`, `approx_vehicle_count`, `info_note`, `primary_contact_id`; new `customer_contacts` table + `CustomerContact` model (driver/supervisor/owner/manager roles + `contacted`/`contacted_at` marker + notes) with `accepts_nested_attributes_for`. `customer_type` + transport/approx fields on the shared customer form; a `?type=` segmentation filter on the admin customers index and the `GET /api/v1/staff/customers` API; summary serializer exposes `customer_type`. **Remaining:** native **Android** type display/filter, and the driver/supervisor/owner **contact-editing UI** (ships with B2's per-visit form). The `visit_entries` per-visit capture table is B2.
