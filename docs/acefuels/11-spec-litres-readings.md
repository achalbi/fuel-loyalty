...
> **Implementation status (2026-07-22):** ✅ **Model + service + API shipped, tested.** `transactions` gained `litres`, `selling_price_snapshot`, `gross_amount`, `discount_amount`, `amount_source` (derived/legacy_amount/manual_amount enum) and `product_id`; `fuel_amount` is now the stored-derived **net** ₹. `FuelPricing.current_price` resolves ₹/L from the A5 catalog; `TransactionCreator` takes a `litres` (+ `discount_amount`) path that snapshots the price, computes `gross`/`net`, and rejects `fuel_price_unset` / negative-net; the ₹ path still works (marked `manual_amount`). `PointsCalculator` gains a by-litres mode and `reward_settings.reward_basis` + `litres_per_reward_unit`. Existing ₹-only rows backfilled as `legacy_amount`. The staff transactions API + web controller accept `litres`/`discount_amount`. **The FSM litres/discount capture *form* (both surfaces) ships with B2** (per-visit CustomerDetailsEntry); the per-nozzle `last_reading` cache ships with D1. Covered by service + model tests.
