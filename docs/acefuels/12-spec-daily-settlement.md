...
15. **The pump chooser is the FSM's way out of a pumpless draft.** Rule 14 deliberately refuses to guess, so an FSM posted day-by-day (dated assignments, no standing pump) who opens the sheet for a date they were not posted to gets `fuel_pump: null` and no nozzle rows. Both surfaces therefore carry a pump + business-date chooser that reloads the draft, and neither may render an empty nozzle grid in silence: it says whether no pump is assigned for that date (pick one and reload) or the named pump has no active nozzles (a setup job for the manager). An empty grid with no rows and no explanation is a dead end — there is nothing to type into and nothing to submit.
...
- No nozzle rows to render: distinguish the two causes and say which (rule 15). `fuel_pump: null` means the caller has no assignment for that business date — the chooser is the fix. A resolved pump with an empty grid means that pump has no active nozzles — a manager has to add them. Silence on either leaves the FSM on a sheet they cannot fill in.
