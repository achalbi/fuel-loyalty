...
> **Superseding note — daily pump assignments (2026-08-19):** S-MYPUMP as originally specified (My Pump hidden from staff entirely) no longer holds. Once per-day assignments landed, "My Pump" came back as a **same-day self-service** screen: `UserPolicy#manage_pump?` is **self-only for both roles** (`record == user && (admin? || staff?)`), and `#assign_pump?` stays admin-only for assigning *someone else's* pump. An admin sees My Pump for the same reason a staff member does — admins are first-class operators here (`TransactionPolicy`/`VisitEntryPolicy`/`DailySettlementPolicy` all resolve `admin? || staff?`), and with nozzle mode on `TransactionCreator` refuses to record a transaction for anyone without a pump + nozzle. It sits in the shared **Access** nav section, not under Admin.
>
> The assignment is now **always for today**, for admins as well as staff:
>
> - The date picker is gone from the web screen and the API resolves the date server-side. A **write** with any other date is refused (`422 daily_assignment_only`); a **read** serves today regardless, because the payload echoes `assignment_date` — that lets a client holding a stale date (an app left open past midnight, or a build from before this rule) correct itself instead of erroring.
> - Rationale: transactions and visit entries snapshot their pump at capture time, so a **back-dated** override cannot move anything already recorded — it only looks as though it did. Forward-dating is roster planning, which belongs to the admin's Assign Pump screen.
> - That admin screen (A10) still plans ahead but **refuses past dates** (`422 past_assignment_date`), for the same reason.
> - Android matches: the self-service screen sends no date at all and re-reads the clock on each load; the date picker remains only on the admin's assign-a-staff-member flow.
...
- **S-MYPUMP:** gate the "My Pump" row in `AccountScreen.kt` (`:123-131`) behind `isAdmin` (the flag already exists at `AccountScreen.kt:71`). Remove the transaction screen's `onSetupPump → Routes.MY_PUMP` affordance for staff (`AppRoot.kt` transaction block); when a staff member has no pump assigned, show "No pump assigned — ask your manager" instead of a link to My Pump. The `MyPumpScreen`/`MyPumpViewModel` can stay in-tree but is only reachable by admins (or be removed entirely if admins assign via the users screen).
...
- [ ] `UserPolicy#manage_pump?` returns false for staff; `GET/PATCH /api/v1/my_pump` and the web `my_pump` route are gone (or 403 for staff).
- [ ] "My Pump" no longer appears in the web nav or the Android staff account screen; it appears for admins only if retained.
