...
> **S-MYPUMP regression + restore (2026-08):** commit `2142f53` ("Add daily pump assignments and customer filters") widened `UserPolicy#manage_pump?` to `user&.admin? && record == user || user&.staff? && record == user`, which silently re-opened My Pump to staff — the exact inversion this spec exists to fix — and added an admin-only assignment-date picker on top. Both are now reverted:
> - `UserPolicy#manage_pump?` is `user&.admin? && record == user` again (write capability).
> - New `UserPolicy#read_pump?` (owner: admin **or** staff) gates `GET /api/v1/my_pump` only. This split is deliberate: the native `TransactionViewModel` and `VisitEntryViewModel` both fetch that endpoint on init to learn which nozzles to offer, so a blanket 403 on read would block staff from recording transactions on Android. Reading your own assignment is not the capability S-MYPUMP forbids; writing it is.
> - The assignment is **pinned to `Date.current`** on every surface. The web date field is gone, `MyPumpsController#assignment_date` and `Api::V1::MyPumpController#assignment_date` both return `Date.current`, `:assignment_date` is no longer a permitted param, and the Android `MyPumpViewModel` stops transmitting a device-local date on the self route. A supplied `assignment_date` is **ignored, never rejected** — a 422 on a clock-skewed device would make the screen unopenable.
> - The web nav link moved out of the shared "Access" block into the **Admin** branch (staff no longer hold the permission, so a shared block rendered an odd empty section for them). This was the client's original complaint: the link sat at the bottom of the long admin menu.
